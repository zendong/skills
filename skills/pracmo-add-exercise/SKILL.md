...
3. 🔴 STOP：只要存在任意命名练习册，就必须列出候选并让用户明确选择，不要根据练习主题自行推断，用户确认前不得继续创建。默认册仍是可选项。
...
一组题目必须让用户学到**具体知识**，而不是练「怎么思考」。本节每一条都与图片门禁同级：任何一条不满足，都不得交付，也不得创建。

- **一条可命名的知识主线**：每个练习包围绕一条真实的、能命名的知识主线组织（例如「一克钠约等于二点五克食盐」）。写完先自问：用户做完能不能用一句话说出自己学到了什么？说不出，这就是空洞内容。
- **禁止用元认知操练冒充知识**：把「观察与评价的区别」「记录怎么写」「提问技巧」「先想再答」这类方法题当作一整包的内容，是最典型的空洞练习。方法只能用来承载知识，不能替代知识本身。
- **六问分工**：一个练习的题组建议依次覆盖：先猜（多数人会答错的判断）→ 机制（为什么会这样）→ 数字或换算（可以算一遍）→ 常见误区（流行说法错在哪）→ 换场景迁移 → 一句话带走（能复述、能教别人）。同一个考点不要用同义改写凑满题数。
- **干扰项必须是真实误解**：错误选项要来自「很多人真的会这么想」，并且让答错的人知道自己错在哪。禁止一眼就能排除的稻草人（例如把「立刻换掉全部家具」写成错误项），也禁止用夸张、荒谬或与题设无关的选项凑数。
- **可复述的锚点**：每组至少给一个数字、换算、机制或命名概念，写在题干或解析里。解析除了说明对错，还要补一句能记住的结论。
- **不靠猎奇与焦虑**：不用夸张、绝对化、恐吓式表达制造吸引力；健康、金融、法律等内容只给一般信息与判断方法，不做个体化建议或承诺。
- **内容要能独立开始**：题设、必要背景、对话与数字都由 AI 生成并放在练习内，不要求用户先找材料。

### 事实依据台账

事实性断言（数字、换算、机制、机构结论、日期、人物原话、专业关系）必须逐条落到**实际打开过的来源**上，而不是模型印象。

- 每条断言登记：来源 URL、原文摘录、适用边界，并对应到具体题号；台账随内容一起交付。
- 搜索摘要、转载、二手复述和模型参数记忆都不能作为唯一依据；搜索只用于定位来源，找到后必须实际打开。
- 找不到可靠来源时只有三条退路：删掉该断言、改成不带事实断言的题，或者向用户索取材料。不得用「研究表明」这类无法追溯的措辞蒙混。
- 时效性内容（价格、行情、政策、模型版本、工具清单、排名）默认不写；确需写入时以实际打开的一手页面为准，并写清核对日期。
- 交付前逐条自检：每组有一条命名主线；六题之间没有同义改写；每个错误选项都是真实误解；至少有一个可复述锚点；所有事实断言都在台账里有出处；没有无法取证却保留的断言。
...
🛑 创建前逐项确认：目标甲程与练习册来自本次会话刚读取或刚创建的响应、所有适用门禁已通过、用户已授权实际创建——任一不满足立即停止，不创建。
...
- 创建甲程，或调用任何甲程创建/`import`/`with-exercise` 接口。
- 用模型参数记忆、搜索摘要或未实际打开的转载充当事实依据。
- 对生成图做本地排版/合成（用户明确要求时除外并记录）。
- 为通过校验虚填 `review`，或跳过 compress → 复核 → validate 流程。
- 绕过 `images finalize` 手工拼 URL；省略 `collectionId` 静默回退默认册。
- 从公开详情反推答案；在门禁未通过时执行创建。
