...
> **边界**：本文件只讲**同一区域内部**的重提（如国内 pre↔prod、换账号）。国内与海外的账号、内容与图片资产互不相通——不存在跨区迁移，也不要把一个区域的内容搬到另一个区域。

提交成功 ≠ 可以结束。创建后要**回读核对**归属、题量、答案与解析，同区内重提既有练习册时还要处理 ID、图片和前向兼容。
