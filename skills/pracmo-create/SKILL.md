...
当用户提供网页、B 站链接、PDF、本地文件、Markdown 长文、截图或明显资料内容时，资料只作为 Agent 理解和出题的输入来源。当前主流程不要求后端抓取资料、转换 PDF、抽字幕、拼 prompt 或生成题目。
...
只有题目本身必须展示图片时，才使用 `pracmo-oss-upload.sh --category practice-assets` 上传图片资产，并把 HTTPS OSS URL 写入题干或选项 Markdown。不要上传资料原文；请求体只保留题目需要的 `evidenceRef` / `evidenceSnippet`。
...
- 如果本次基于资料创建，Agent 已经读完资料并明确题目来源边界；请求体只提交完整 questions。
...
- `source`：用户提供 Markdown、文章、转录稿或资料。题目必须以资料为事实锚点；外部常识只能辅助解释，不能作为答案依据。
...
- 请求体只提交甲程归属、练习元信息和完整 questions；不要加入与题目创建无关的资料字段。
...
```markdown
![供需曲线示意图](https://bucket.oss-cn-hangzhou.aliyuncs.com/practice-assets/diagram.png)
```

提交前逐题自检：

- 所有图片 URL 必须以 `https://` 开头，并位于上传脚本返回的账号 OSS 前缀下。
