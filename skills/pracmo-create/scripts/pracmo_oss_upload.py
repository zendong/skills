#!/usr/bin/env python3
"""Upload one compressed private learning-track image with Pracmo OSS STS."""
...
def build_object_key(category_prefix, request_id, path, *, stamp=None, nonce=None):
    if not REQUEST_ID_PATTERN.fullmatch(request_id):
        raise ValueError("clientRequestId format is invalid")
    stamp = stamp or datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    nonce = nonce or uuid.uuid4().hex[:10]
    return f"{category_prefix}{request_id}/{stamp}-{nonce}-{safe_name(path)}"
...
def upload(path, request_id, api_key, base):
    ...
    if not 0 < size <= MAX_IMAGE_BYTES:
        raise ValueError("learning track image must be 512 KiB or smaller")
    content_type = mimetypes.guess_type(path.name)[0] or "application/octet-stream"
    if content_type == "image/jpg":
        content_type = "image/jpeg"
    if content_type not in {"image/png", "image/jpeg", "image/webp"}:
        raise ValueError(f"unsupported learning track image type: {content_type}")
    ...
    category_prefix = str((config.get("objectKeyPrefixes") or {}).get("learningTrackAssets", "")).strip()
    if not bucket_name or not endpoint or not account_prefix or not category_prefix:
        raise RuntimeError("OSS config is missing bucket, endpoint, or learningTrackAssets prefix")
    if not category_prefix.startswith(account_prefix):
        raise RuntimeError("learningTrackAssets prefix is outside the account OSS prefix")
    ...
    object_key = build_object_key(category_prefix, request_id, path)
