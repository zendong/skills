...
die() { echo "pracmo-add-action: $*" >&2; exit 1; }
require_key() {
...
add_action() {
  ...
  response=$(curl -sS -w $'\n%{http_code}' -X POST \
    ...
    --data-binary @"$file" "${BASE_URL}/learning-tracks/${track_id}/actions") || die "curl 失败"
  code="${response##*$'\n'}"; response="${response%$'\n'*}"
  emit_response "$response" "$code"
}
...
case "${1:-}" in
  ...
  add-action)
    [[ $# -eq 3 ]] || die "用法: $0 add-action <trackId> <json-file>"
    add_action "$2" "$3"
    ;;
  *) die "用法: $0 list-tracks [keyword] | add-action <trackId> <json-file>" ;;
esac
